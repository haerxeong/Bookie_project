...
class MissionRepository {
    ...
    fun addMission(userId: String, mission: Mission) {
        val missionRef = userRef.child(userId).child("mission").push() // USERID -> MISSIONS
...
//        val missionRef = userRef.child(userId).child("mission")
    ...
    fun updateMission(userId: String, missionId: String, updatedMission: Mission) {
        userRef.child(userId).child(missionId).setValue(updatedMission).addOnCompleteListener {
            if (it.isSuccessful) {
                ...
                Log.e("MissionRepository", "Error updating mission", it.exception)
            }
        }
    }
    ...
    fun deleteMission(userId: String, missionId: String) {
        userRef.child(userId).child(missionId).removeValue().addOnCompleteListener {
            if (it.isSuccessful) {
